...
const getTools = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/mcp/tools`); // Request goes to /api/mcp/tools
    if (!response.ok) {
...
const executeTool = async (toolId, action, params) => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/mcp/execute`, { // Request goes to /api/mcp/execute
      method: "POST",
      headers: {
